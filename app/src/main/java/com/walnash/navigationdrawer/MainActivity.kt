...
fun NavigationHost(navController: NavController, viewModel: MainViewModel) {
    NavHost(
        ...
        composable(Screens.DrawerScreens.Home.route) {
            Log.d("TEST","navigate - home")
            Home(viewModel = viewModel)
        }
        composable(Screens.HomeScreens.Favorite.route) {
            Log.d("TEST","navigate - favourite")
            Favorite(viewModel = viewModel)
        }
        composable(Screens.HomeScreens.Notification.route) {
            Log.d("TEST","navigate - noti")
            Notification(viewModel = viewModel)
        }
        composable(Screens.HomeScreens.MyNetwork.route) {
            Log.d("TEST","navigate - myNetwork")
            MyNetwork(viewModel = viewModel)
        }
        composable(Screens.DrawerScreens.Account.route) {
            Log.d("TEST","navigate - account")
            Account(viewModel = viewModel)
        }
        composable(Screens.DrawerScreens.Help.route) {
            Log.d("TEST","navigate - help")
